...
import batchAssociateService from '../../batchAssociates/batchAssociate.service';
...
test('That axios is called with correct path parameters', async () => {
  ...
  const res = await batchAssociateService.getAssociates(testPath);

  expect(res).toEqual('some data');
  expect(axios.get).toHaveBeenCalledTimes(1);
  expect(axios.get).toHaveBeenLastCalledWith(URI + '/batch/YYMM-mmmDD-Stuff', {
...
test('That we can handle receiving an error from axios', async () => {
  axios.get = jest
    ...
    .mockRejectedValueOnce(
      'If you see this error, the test is functioning correctly'
    );
